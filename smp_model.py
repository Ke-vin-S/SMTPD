...
#         self.decoder_layers = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=d_model, nhead=nhead), num_layers=num_decoder_layers)
#         self.fc = nn.Linear(d_model, output_dim)
#
#     def forward(self, src, trg):
#         # src: 输入的高维特征，shape为 (seq_len, batch_size, input_dim)
#         # trg: 输入的目标序列，shape为 (trg_len, batch_size)
#         trg_embedded = self.embedding(trg) * torch.sqrt(self.d_model)
#         output = self.decoder_layers(trg_embedded, src)
#         output = self.fc(output)
#         return output
#
#
# # 定义Transformer模型带有Decoder
# class TransformerModel(nn.Module):
#     def __init__(self, input_dim, output_dim, num_heads, hidden_dim, num_encoder_layers,
#                  num_decoder_layers):
...
#
#     def forward(self, input, tgt_seq):
#         # 输入维度：[sequence_length, batch_size, input_dim]
#         embedding = self.embedding(input)
#         embedding = embedding.unsqueeze(0)
#         # 转换为Transformer要求的维度：[sequence_length, batch_size, hidden_dim]
#         embedding = embedding.permute(1, 0, 2)
#         # 使用Transformer编码器进行序列转换
#         encoder_output = self.transformer_encoder(embedding)
#         # 使用Transformer解码器生成输出序列
#         tgt_seq = tgt_seq.permute(1, 0)
#         decoder_output = self.transformer_decoder(tgt_seq, encoder_output)
#         # 转换回预测输出维度：[batch_size, sequence_length, output_dim]
#         output = self.linear(decoder_output.permute(1, 0, 2))
#         return output


class bili_model(nn.Module):
    def __init__(self):
        super(bili_model, self).__init__()

        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])

        self.tokenizer = BertTokenizer.from_pretrained('../bert_chinese')
        self.bert_model = BertModel.from_pretrained('../bert_chinese').to(device)
        self.conv = nn.Conv2d(2, 1, 1)
        self.conv.weight.data.normal_(1 / 2, 0.01)
    ...
    def forward(self, img, texts, meta):
        ...
        text_features = text_features.permute(0, 2, 1, 3).squeeze()
...
class bili_model_cut(nn.Module):
    def __init__(self):
        super(bili_model_cut, self).__init__()
        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])
        ...
        self.img_classifier = nn.Sequential(nn.Linear(2048, 128))
        self.text_classifier = nn.Sequential(nn.Linear(768, 128))
        self.meta_classifier = nn.Sequential(nn.Linear(8, 128))
    ...
    def forward(self, img, texts, meta):
        ...
        out = torch.cat([img_features, text_features, meta_features], 1)
        return out


class bili_model_new(nn.Module):
    def __init__(self):
        super(bili_model_new, self).__init__()

        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])

        self.tokenizer = BertTokenizer.from_pretrained('../bert_chinese')
        self.bert_model = BertModel.from_pretrained('../bert_chinese').to(device)
        self.ft_model = ft.load_model('../ft_model/cc.zh.300.bin')
        self.conv = nn.Conv2d(2, 1, 1)
        self.conv.weight.data.normal_(1 / 2, 0.01)
    ...
    def forward(self, img, texts, meta):
        ...
        for text in texts:
            with torch.no_grad():
                ...
                # fasttext feature
                ft_features = ft_feature(text, self.ft_model)
            text_features = torch.cat([bert_features, ft_features], 1)
            text_features_list.append(self.text_classifier(text_features))
        text_features = torch.stack(text_features_list, 1).unsqueeze(3)
        text_features = self.conv(text_features).permute(0, 2, 1, 3).squeeze()
        #
...
class bili_model_new_drop(nn.Module):
    def __init__(self):
        super(bili_model_new_drop, self).__init__()
...
class bili_model_new_cut(nn.Module):
    def __init__(self):
        super(bili_model_new_cut, self).__init__()
        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])
...
class bili_multiout(nn.Module):
    def __init__(self, seq_length):
        super(bili_multiout, self).__init__()
        self.text_num = 2
        self.meta_num = 7
        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])
        self.tokenizer = BertTokenizer.from_pretrained('../bert_chinese')
...
class bili_transformer(nn.Module):
    def __init__(self, seq_length):
        super(bili_transformer, self).__init__()
        self.text_num = 2
        self.meta_num = 6
        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])
        self.tokenizer = BertTokenizer.from_pretrained('../bert_chinese')
        ...
        self.decoder = nn.TransformerDecoder(decoder_layer=decoder_layer, num_layers=3, norm=True)

    def forward(self, img, texts, meta, tgt_seq):
        # num
        meta_features = self.meta_classifier(meta)
        # picture
        with torch.no_grad():
        ...
        out = self.decoder(tgt_seq, x)
        return out


# use features combined with 1day_pop input
class bili_lstm1(nn.Module):
    def __init__(self, seq_length, batch_size):
        super(bili_lstm1, self).__init__()
        self.text_num = 2
        self.meta_num = 7
        self.img_feature = nn.Sequential(*list(models.resnet101(pretrained=True).children())[:-1])
        ...
        self.lstm = nn.ModuleList([nn.LSTMCell(input_size=self.vec_length, hidden_size=self.hidden_size)
...
# use features combined with 1day_pop input
class youtube_lstm1(nn.Module):
    def __init__(self, seq_length, batch_size):
        super(youtube_lstm1, self).__init__()
        self.text_num = 5
        self.meta_num = 5
        ...
        self.ft_model = {"zh": ft.load_model('../ft_model/cc.zh.300.bin'),
                         "en": ft.load_model('../ft_model/cc.en.300.bin'),
                         "ko": ft.load_model('../ft_model/cc.ko.300.bin'),
                         "ja": ft.load_model('../ft_model/cc.ja.300.bin'),
                         "hi": ft.load_model('../ft_model/cc.hi.300.bin'),
                         "ru": ft.load_model('../ft_model/cc.ru.300.bin'),
                         }
        self.conv = nn.Conv2d(self.text_num, 1, 1)
        self.conv.weight.data.normal_(1 / self.text_num, 0.01)
        ...
        #     nn.BatchNorm1d(256, affine=False),
        #     nn.ReLU(),
        #     nn.Linear(256, self.vec_length),
        # )
        self.vec_length = 768
        self.seq_length = seq_length
        self.hidden_size = 768
        self.c0 = torch.zeros(self.batch_size, self.hidden_size).to(device)
        self.LSTMCell_list = nn.ModuleList([nn.LSTMCell(input_size=self.vec_length, hidden_size=self.hidden_size)
                                            for _ in range(self.seq_length)])
        self.MLPo_list = nn.ModuleList([nn.Sequential(
    ...
    def forward(self, img, texts, meta):
        ...
        for text in texts:
            with torch.no_grad():
                ...
                bert_features = bert_feature(text, self.bert_model, self.tokenizer)
                # fasttext feature
                ft_zh = ft_feature(text, self.ft_model['zh'])
                ft_en = ft_feature(text, self.ft_model['en'])
                ft_ko = ft_feature(text, self.ft_model['ko'])
                ft_ja = ft_feature(text, self.ft_model['ja'])
                ft_hi = ft_feature(text, self.ft_model['hi'])
                ft_ru = ft_feature(text, self.ft_model['ru'])
            text_features = torch.cat([bert_features,
                                       ft_zh, ft_en, ft_ko, ft_ja, ft_hi, ft_ru,
                                       ], 1)
        ...
        h = feature_vector
        c = self.c0
        out = torch.zeros(self.batch_size, self.seq_length).to(device)
        for i in range(self.seq_length):
            h, c = self.LSTMCell_list[i](feature_vector, (h, c))
            o_i = self.MLPo_list[i](h)
            # print(out)
            # print(o_i)
            out[:, i] = o_i.squeeze(1)
        return out
...
class youtube_lstm3(nn.Module):
    ...
    def forward(self, img, texts, meta, cat):
        ...
        #feature_vector = torch.sum(feature_vector, dim=1)

        # Print feature_vector shape for debugging
        # print("Feature vector shape:", feature_vector.shape)
        #
        # # Ensure feature_vector has correct shape
        # feature_vector = feature_vector.view(self.batch_size, -1)

        # 将特征排列以适配 MultiheadAttention 的输入
        # features = features.view(self.batch_size, self.seq_length, -1).permute(1, 0, 2)  # shape: (seq_length, batch_size*4, embedding_dim)

        # attn_output, _ = self.multihead_attn(features, features, features)
        # feature_vector = attn_output.mean(dim=0).to(device)
